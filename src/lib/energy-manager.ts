...
export class EnergyManager extends EventEmitter {
   ...
   */ public registerDevice(
    ...
    deviceLogger.info("Registering new device", {
      id,
      name,
      type,
   ...
   */ public removeDevice(id: string): boolean {
    ...
    if (this.mqtt.isClientConnected()) {
      ...
      deviceLogger.debug("Unsubscribing from device status topic", {
        topic: statusTopic,
      });
   ...
   */ public async sendCommandToGroup(
    groupName: string,
    command: CommandType,
    payload?: any,
  ): Promise<void> {
    ...
    if (deviceIds.length === 0) {
      groupLogger.warn("Group command skipped - group has no devices", {
        groupName,
      });
      return;
    }
  ...
  public getGroupStatistics(groupName: string): GroupStatistics {
    ...
    statistics.averageBatteryLevel =
      batteryCount > 0 ? batterySum / batteryCount : 0;
   ...
   */ public setTopicPrefix(prefix: string): void {
    ...
    const resubscribe =
      prefix !== this.topicPrefix && this.mqtt.isClientConnected();
    ...
    if (resubscribe) {
      ...
      this.subscribeToAllDeviceStatuses().catch((err) => {
        this.logger.error(
          "Failed to resubscribe to status topics with new prefix",
          err,
        );
      });
    }
   ...
   */ private async subscribeToAllDeviceStatuses(): Promise<void> {
    if (!this.mqtt.isClientConnected()) {
      this.logger.debug(
        "Skipping subscription to device statuses - not connected to MQTT broker",
      );
      return;
    }
   ...
   */ private startStatusCheck(): void {
    ...
    this.logger.debug("Status checking started", {
      ...
      nextCheckAt: new Date(
        Date.now() + this.statusUpdateInterval,
      ).toISOString(),
    });
  }
